				...
				{items.map((item) => (
					<Collapsible className="group/collapsible" defaultOpen={
                    							...
                    							item.subItems?.some((i) => !!i.isActive)
                    										...
                    										<SidebarMenuSub>
                    											{item.subItems?.map((subItem) => (
                    												<SidebarMenuSubItem key={subItem.title}>
                    													<SidebarMenuSubButton isActive={subItem.isActive} render={<Link to={subItem.path || "#"} />}>{subItem.icon}<span>{subItem.title}</span></SidebarMenuSubButton>
                    												</SidebarMenuSubItem>
                    											))}
                    										</SidebarMenuSub>
                    									</CollapsibleContent>
                    								...
                    								<SidebarMenuButton isActive={item.isActive} render={<Link to={item.path || "#"} />}>{item.icon}<span>{item.title}</span></SidebarMenuButton>
                    							)}</Collapsible>
				))}
			</SidebarMenu>
		</SidebarGroup>
